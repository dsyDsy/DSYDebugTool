...
@objc public class CocoaDebugSettings: NSObject {
    ...
    @objc public var serverURL: String? = nil {
        didSet {
            UserDefaults.standard.set(serverURL, forKey: "serverUEL_CocoaDebug")
            UserDefaults.standard.synchronize()
        }
    }
    ...
    @objc public var logCount: Int  = 500
    //{
//        didSet {
//            _OCLogStoreManager.shared().logCount = logCount
//            _OCLogStoreManager.shared().initializeContainer()
//        }
//    }
    @objc public var httpCount: Int  = 500  //{
//        didSet {
//            _HttpDatasource.shared().httpCount = httpCount
//            _HttpDatasource.shared().initializeContainer()
//        }
//    }
    ...
    /// 自定义网络分享处理回调，用于在 UIActivityViewController 中提供自定义入口
    /// 参数：messageBody - 网络请求的详细信息文本
    /// 参数：httpModel - 网络请求模型
    public var customNetworkShareHandler: ((_ messageBody: String, _ httpModel: _HttpModel?) -> Void)? = nil
    
    /// 自定义网络分享入口的标题，默认为 "Custom Share"
    public var customNetworkShareTitle: String = "Custom Share"
    
    /// 自定义网络分享入口的图标，默认为 nil（使用系统默认图标）
    public var customNetworkShareImage: UIImage? = nil
    ...
    private override init() {
        ...
        serverURL = UserDefaults.standard.string(forKey: "serverUEL_CocoaDebug")
